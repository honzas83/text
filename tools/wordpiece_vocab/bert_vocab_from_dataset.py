...
def bert_vocab_from_dataset(dataset,
  ...
  Args:
    ...
    bert_tokenizer_params: The `text.BertTokenizer` arguments relavant for to
      vocabulary-generation: * `lower_case` * `keep_whitespace`, *
        `normalization_form` * `preserve_unused_token`.  See `BertTokenizer` for
        details. You should use the same values for these to both generate the
        vocabulary and build the `BertTokenizer`.
